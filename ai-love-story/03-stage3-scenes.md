...
| **2 · הזירה** | 9 שוטים, 18 שניות — הקרב שלו והתא שלה, משולבים | `@He_arena`, `@She_arena`, `@giant`, `@arena`, `@sketch` | 18s |
...
**מודל:** Seedance 2.5 · **יחס:** 21:9 · **רזולוציה:** 1080p · 9 שוטים, 18 שניות
**Elements:** `@He_arena` · `@She_arena` · `@giant` · `@arena` · `@sketch`
...
[`giant-sheet.prompt.txt`](giant-sheet.prompt.txt) ·
...
| שוט | | אורך |
|---|---|---|
| 1 | החול — גיאוגרפיה **ויחס הגודל** | 2.0s |
| 2 | היא בתא, קרירה, לא נותנת כלום | 1.5s |
| 3 | הריצה — הוא זעיר מול המסה | 1.5s |
| 4 | הטיפוס — הוא עולה על גב הענק דרך ה-X של השרשראות | 2.0s |
| 5 | היא כמעט מסגירה את עצמה — נשימה נעצרת, נטייה של סנטימטרים | 1.5s |
| 6 | ההטלה — הענק נופל, האדמה קופצת | 2.5s |
| 7 | החרב — הוא עומד מעליו ולא מכה | 2.0s |
| 8 | הוא מרים עיניים אליה | 2.0s |
| 9 | **המבט האחד שלה** — סלואו-מושן, פריים מוחזק | 3.0s |
...
**הקסדה לא יורדת.** לענק אין פנים לאורך כל הסצנה — שני חורי עיניים וכתר עור תפור וחלק.
זה גם עיצוב וגם הגנה: פנים שהמודל ממציא בכל שוט הן פנים שמשתנות בכל שוט.

**הוא לא הורג.** הקרב נגמר עם הענק שטוח והחרב נמוכה. אין דם בשום מקום בסרט. מעבר לטעם —
זה מה שהופך אותו לדמות שאפשר להתאהב בה בשש תקופות.
