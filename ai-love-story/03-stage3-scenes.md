...
| **2 · הזירה** | 9 שוטים, 18 שניות — הקרב שלו והתא שלה, משולבים | `@He_arena`, `@She_arena`, `@robot`, `@arena`, `@sketch` | 18s |
...
| 4 | הטיפוס — הוא עולה על גב המכונה דרך ה-X של השרשראות | 2.0s |
| 5 | היא כמעט מסגירה את עצמה — נשימה נעצרת, נטייה של סנטימטרים | 1.5s |
| 6 | ההטלה — האוטומטון נופל, האדמה קופצת | 2.5s |
...
**למה אוטומטון ברונזה ולא מֶק.** רובוט סיינס-פיקשן בזירה עתיקה שובר את העולם ומושך את
המודל לרנדר משטחים מבריקים ואורות ניאון. אוטומטון בסגנון טאלוס נשאר בתוך התקופה,
ומרוויח משהו שלענק לא היה: **מבנה גלוי**. הצלעות, הכבלים והשרשראות בין לוחות הברונזה
הם מה שהוא מטפס עליו — מנגנון הטיפוס נשמר, ועכשיו הוא הגיוני יותר.
