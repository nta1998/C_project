...
**מודל:** Seedance 2.5 · **יחס:** 21:9 · **רזולוציה:** 1080p · 7 שוטים, 18 שניות
**Elements:** `@He_kitchen` · `@She_kitchen` · `@kitchen` · `@olives`
...
[`olives-plate.prompt.txt`](olives-plate.prompt.txt)

## הבדיחה

**הוא** טבח בוויתים. **היא** ברמנית מהבר — ג'ינס, חולצה לבנה, שלייקס אדומים. היא נכנסת
למטבח עם דלי זיתים תעשייתי סגור, וכל מה שהיא עושה נקרא כמו אישה שבאה בשבילו: היא
מחזיקה מבט שנייה יותר מדי, היא נשארת אחרי שהדלי נפתח, והיא מסתכלת עליו ולא על הצלחות.

ואז השורה השלישית מפוצצת את זה. **והוא לא מספיק להוציא מילה.**

| שוט | | אורך |
|---|---|---|
| 1 | הקו בשיא, הוא בעבודה | 2.0s |
| 2 | **היא נכנסת** — דלת מתנדנדת, דלי סגור, שלייקס אדומים. מבט שנשאר שנייה יותר מדי | 2.5s |
| 3 | **"Hey, can you help me open this?"** | 2.5s |
| 4 | הוא פותח — מנגב ידיים, כף על המכסה, פוקק בתנועה אחת | 2.0s |
| 5 | **הוא ממלא שתי צלחות** בלי שביקשו. היא לא עוזרת — היא מסתכלת עליו | 3.5s |
| 6 | **"How old are you?"** — הוא עוצר, מסתובב, **לוקח אוויר, השפתיים נפתחות** | 2.5s |
| 7 | **"I have a sister. You have to meet her."** — היא כבר חוזרת לצלחות. הוא עוד באמצע הנשימה. **קאט** | 3.0s |

## שלושת החוקים שנושאים את הבדיחה

**`THE THREE LINES`** — שלוש שורות, כולן שלה, עם טיים-קוד מדויק לכל אחת. זה הדיבור
היחיד בכל הסרט. קולות הבריגדה נשארים `NEVER intelligible`.

**`HE NEVER GETS A WORD OUT`** — הוא לא אומר מילה בכל הסרט. אחרי השורה השנייה הוא
**לוקח אוויר והשפתיים נפתחות** — והשורה השלישית נוחתת מעל זה לפני שיוצא צליל.
`That failed answer is the joke and it must be visible.`

**`SHE IS TAKEN WITH HIM, AND SHE IS NOT HIDING IT`** — זה ה-setup, וחייבים לשחק אותו
**לגמרי ברצינות**: `no wink and no irony anywhere before the last line`. אם המודל ירמז
מראש שזו בדיחה, הפאנץ' מת.

## פרטים קטנים שנעולים

**השלייקס האדומים הם הצבע הרווי היחיד בחדר.** נכתב גם בבלוק הטכני. בין הפלדה, הלבן
והענבר של הפאס — היא בולטת מהפריים הראשון שהיא בו.

**היא היחידה שלא בבגדי מטבח.** בלי סינר, בלי ז'קט שף, בלי כובע — `she should read
instantly as coming from the bar`.

**הדלי נפתח פעם אחת ובקלות.** הוא לא נאבק בו. `Nothing about it is a struggle` —
זו הנקודה: היא ביקשה משהו קל, והוא שמח לעשות אותו.

**הקאט קשה לשחור על המילה האחרונה.** בבלוק האודיו: אחרי שתי השורות הראשונות החדר
חוזר לעוצמה; אחרי השלישית **הוא לא חוזר** — הסרט נחתך עליה.

## שינוי נכסים

`@bowl` (קערת נירוסטה) הוחלף ב-`@olives` — לוח פרופים אחד שכולל **גם** את הדלי
התעשייתי הסגור **וגם** את שתי צלחות ההגשה השחורות הארוכות. נכס אחד במקום שניים.


---

# סצנה 8 — אחרי הקרדיטים

**מודל:** Seedance 2.5 · **יחס:** 21:9 · **רזולוציה:** 1080p · 8 שוטים, **25 שניות**
**Elements:** `@He_today` · `@She_today` · `@boys` · `@livingroom` · `@game`
**קבצים:** [`scene8-postcredits.prompt.txt`](scene8-postcredits.prompt.txt) ·
[`he-today-sheet-tagged.prompt.txt`](he-today-sheet-tagged.prompt.txt) ·
[`she-today-sheet-tagged.prompt.txt`](she-today-sheet-tagged.prompt.txt) ·
[`boys-sheet.prompt.txt`](boys-sheet.prompt.txt) ·
[`livingroom-plate.prompt.txt`](livingroom-plate.prompt.txt) ·
[`game-screen.prompt.txt`](game-screen.prompt.txt)

## מה הקטע עושה לסרט

חמש התקופות מתגלות בדיעבד כ**שלבים במשחק** ששני ילדים משחקים על השטיח. והיקום שההורים
באמת חיים בו הוא היחיד שבו אף אחד לא היה צריך להילחם — היא פשוט נכנסה למטבח וביקשה
לפתוח דלי זיתים.

הסרט בילה 90 שניות בלגרום להושטת יד בלתי אפשרית להיראות רומנטית. הקטע הזה אומר
שהיומיומי ניצח.

| שוט | | אורך |
|---|---|---|
| 1 | שני הילדים על הכריות מול הטלוויזיה, אור המשחק על הפנים | 2.5s |
| 2 | **"What are you playing?"** — הקטן לא עונה | 2.5s |
| 3 | **המסך** — `HOW MOM AND DAD MET`, שישה שלבים, `UNIVERSE 047`… | 4.5s |
| 4 | חזרה לילדים. אף אחד לא חושב שזה מיוחד. זה יום שלישי | 2.0s |
| 5 | **היא נכנסת** — "Boys. Dinner." · "This universe is the best one…" | 4.5s |
| 6 | **הוא נכנס** — "In every universe, I would fight for you." | 3.0s |
| 7 | **"Except this one."** | 2.0s |
| 8 | **"That's why it's the best one."** — הילדים עדיין משחקים. פריים מוחזק | 4.0s |

## שתי החלטות טכניות

**מסך המשחק הוא נכס תמונה נפרד.** מודלי וידאו לא מייצרים טקסט אמין. `@game` נוצר
ב-**GPT Image 2** — שם הטקסט אמין — ומוזן כרפרנס. בפרומפט הוידאו יש חוק `THE SCREEN`
שקובע שהמסך משוחזר **בדיוק כמו ברפרנס**, בלי לגלול, בלי גיימפליי, בלי טקסט שלא בו.
אם הטקסט בכל זאת יתעוות בגנרציה — **תחליף את המסך בעריכה**. זו ממילא העבודה הנכונה.

**25 שניות ולא 18.** יש כאן שישה חילופי דיבור. דחיסה ל-18 הורגת את הפאנץ'.

## שלושת החוקים

**`THE JOKE IS NEVER SIGNALLED`** — אף אחד לא קורץ, אף אחד לא מחייך על השורה של עצמו,
ואף הורה לא מסתכל למצלמה. "Except this one" נאמר **בחיבה ועניינית**, לא כעקיצה.
התשובה שלו שקטה ובטוחה, לא מנצחת. `The scene must never announce that it is funny.`

**`THE BOYS DO NOT CARE`** — אף ילד לא מגיב לאף שורה של ההורים. הם לא הקהל של החילופין
האלה. אנחנו כן.

**`AGE HIM / AGE HER`** — אתם אותם שניים, חמש-עשרה שנה אחר כך. הפרומפט אומר במפורש
`the change is age, not identity` ו-`he reads as forty, not sixty`. זו נקודת הכשל הצפויה —
מודלים נוטים להזקין יותר מדי ולהחליף פנים.

## הערה על הילדים

`@boys` נוצר מ-`@He_today` ו-`@She_today` כהורים, עם הנחיה מפורשת מה לקחת מכל אחד:
מבנה העין והשיער ממנו, האף והשפתיים ממנה, והעיניים הכחולות שלה **רק לקטן**.
הגילים (9 ו-6) והבגדים שרירותיים — תגיד לי את הגילים והמראה האמיתיים ואעדכן.
